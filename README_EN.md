...
- [dsh-context](https://github.com/bowenliang123/dsh-context): shows the per-request token composition of system prompts, tool schemas, messages, injections, replies, and tool results in a Web UI Context panel and `/context` command, including compaction, pruning, and cache-hit signals; Apache-2.0 and npm / Release `0.24.1`, with DSH `0.1.1` multimodal attachment views plus labeled Raw / Sent / Token image facts and wrapping full filenames, peer dependencies still starting at `^0.1.0-rc.7`, and host and client tests. It requires no external service, though the UI checks npm for a newer version at most once per hour.
