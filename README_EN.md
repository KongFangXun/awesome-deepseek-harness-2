...
- [dsh-context](https://github.com/bowenliang123/dsh-context): shows the per-request token composition of system prompts, tool schemas, messages, injections, replies, and tool results in a Web UI Context panel and `/context` command, including compaction, pruning, and cache-hit signals; Apache-2.0 and npm / Release `0.17.0`, built against DSH `0.1.0-rc.8` dependencies with host and client tests. It requires no external service, though the UI checks npm for a newer version at most once per hour.
