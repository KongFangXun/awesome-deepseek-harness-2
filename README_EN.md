...
- [oh-story-dsh](https://github.com/worldwonderer/oh-story-dsh): brings 13 novel-writing Skills, seven specialist Roles, and a short-drama production workflow into native DSH Sessions, approvals, and a three-column creative workbench. MIT, npm / Release `0.1.2`, Node.js 24+, and peer dependencies starting at DSH `0.1.1-rc.1`; the Release reports 34 automated tests and an end-to-end official Web UI run, while the build excludes upstream login / CDP scrapers and the standalone dashboard. The plugin does not read model credentials or open its own listener, but it can read and write creative-project files and invoke specialist Roles within the calling Agent's visible tools, and it adds a DSH Web Server file route protected by Host, Origin, Session, path-containment, and optimistic-concurrency checks. First released on August 21 and explicitly verified only on DSH rc.1, it still lacks long-term maintenance and independent-usage evidence, so it is marked Early.

### Context, Sessions, and Input

- [dsh-context](https://github.com/bowenliang123/dsh-context): shows the per-request token composition of system prompts, tool schemas, messages, injections, replies, and tool results in a Web UI Context panel and `/context` command, including compaction, pruning, and cache-hit signals; Apache-2.0 and npm / Release `0.25.3`, extending the DSH `0.1.1` multimodal attachment and Raw / Sent / Token image facts with unified section cards for expanded details, whole-session image counts, collapsed-message Image chips, and corrected weekend off-peak pricing. Peer dependencies still start at `^0.1.0-rc.7`, with host and client tests. It requires no external service, though the UI checks npm for a newer version at most once per hour.
...
- [dsh-im](https://github.com/xmanrui/dsh-im): manages Feishu, WeChat, DingTalk, WeCom, QQ, Slack, Telegram, Discord, WhatsApp, and AI Office in one DSH Bundle, with multiple bots, streaming replies, workspace / Session binding, and remote approvals. MIT, npm `1.0.2` / Git tag `v1.0.2`, and Node.js 22.19+, with 89 test files and a package-verification script; this release makes both Feishu REST and WebSocket traffic honor explicit HTTP(S) proxy settings. Secrets and Tokens are submitted only to the local Harness Host, and management RPC is loopback-browser-only by default. Runtime use continuously receives and sends external messages and lets trusted chat users invoke models and tools; outside Telegram's optional direct-message allowlist, platform-visible users can run commands, while workspace / Session listings may expose local paths and sensitive metadata, so access must be limited to trusted users.
...
- [dsh-market](https://github.com/dsh-market/dsh-market): in-DSH plugin market for browsing, searching, installing, updating, and uninstalling projects listed by `awesome-dsh-plugin`; MIT and `v1.18.1`, with post-install/update Profile compatibility checks and one-click rollback for a single plugin, plus fixes for category-collapse state that cannot persist, a false positive on its own optional peer, and proxy propagation into Git installs. Build scripts are blocked by default, the install endpoint accepts same-origin POST only, and updates are blocked while an Agent runs and preserve a bootable state on failure, but directory inclusion is still not a security endorsement.
