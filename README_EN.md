...
- [dsh-market](https://github.com/dsh-market/dsh-market): in-DSH plugin market for browsing, searching, installing, updating, and uninstalling projects listed by `awesome-dsh-plugin`; MIT and `v1.17.1`, with post-install/update Profile compatibility checks and one-click rollback for a single plugin. Build scripts are blocked by default, the install endpoint accepts same-origin POST only, and updates are blocked while an Agent runs and preserve a bootable state on failure, but directory inclusion is still not a security endorsement.
