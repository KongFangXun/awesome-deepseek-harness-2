...
The latest verified official GitHub developer preview is [`0.1.2-alpha.1`](https://github.com/deepseek-ai/deepseek-harness/releases/tag/dsh-v0.1.2-alpha.1), while npm `latest` remains `0.1.1-rc.2`. This Alpha adds per-subagent model configuration, ACP automation, and multimodal / persistent-terminal fixes; DeepSeek adapter requests include enabled plugin package names and versions by default (configurable off), while incremental session-log upload remains opt-in and off by default. DSH versions shown for individual projects below represent the development or test baseline claimed by their authors and should not be treated as automatic compatibility with the latest preview.
...
- [sofagent](https://github.com/KongFangXun/sofagent): a commit-time governance harness for AI coding agents; MIT, listed on the GitHub Marketplace as an Action, also available as an MCP server. 24 deterministic rules audit git diffs locally (secrets, out-of-scope edits, blind modifications, prompt-injection traces) with zero model calls; an HMAC-chained audit history is tamper-evident. Ships a 9-plugin cordis family under `engine/dsh-plugins/` (audit / gate / rollback / inject / ontology / evolve / commons / daemon / fde), distributed via SkillHub (`skillhub install cordis-plugin-sofagent-*`). Rule semantics are decoupled from official DSH releases - it attaches at the repository side rather than the Profile side.
