...
- [dsh-context](https://github.com/bowenliang123/dsh-context): shows the per-request token composition of system prompts, tool schemas, messages, injections, replies, and tool results in a Web UI Context panel and `/context` command, including compaction, pruning, and cache-hit signals; Apache-2.0 and npm / Release `0.26.1`. This version adds Total / Diff modes to Context History, labels Skill loads, provides a full-height Focus Mode, and fixes a React #310 crash that could occur when the first projection data or event arrived. Peer dependencies still start at `^0.1.0-rc.7`, with host and client tests. It requires no external service, though the UI checks npm for a newer version at most once per hour.
...
- [dsh-win32](https://github.com/sjh9714/dsh-win32): diagnostics and safe repairs for current DSH on Windows; MIT and npm / Release `v0.16.0`, with the latest CI passing. Default `setup` no longer replaces the official stack: it verifies the built-in persistent PowerShell and Workspace Write packages, repairs only provably broken `koffi` loading, and can create a desktop shortcut. `doctor` is read-only, while `fix` only repairs an installed `koffi` version known to be broken or failing a real load. The old Git Bash / BusyBox presets moved behind explicit `setup --legacy`, where Git Bash still requires `danger-full-access`; the tool does not automatically install Git, PowerShell, BusyBox, WSL, or another DSH bundle.
...
- [dsh-im](https://github.com/xmanrui/dsh-im): manages Feishu, WeChat, DingTalk, WeCom, QQ, Slack, Telegram, Discord, WhatsApp, and AI Office in one DSH Bundle, with multiple bots, streaming replies, workspace / Session binding, and remote approvals. MIT, npm `1.3.0` / Git tag `v1.3.0`, and Node.js 22.19+, with 100 test files and a package-verification script; all nine built-in channels can now deliver ordinary files in both directions, with unified inbound prefetching, workspace staging, path protection, failure messages, and end-of-turn cleanup. Existing and new WhatsApp bots default to self-chat only and can switch to selected contacts or open responses. Secrets and Tokens are submitted only to the local Harness Host, and management RPC is loopback-browser-only by default. Runtime use continuously receives and sends external messages and files and lets authorized chat users invoke models and tools; each platform still defines file type, count, size, and access policy, while workspace / Session listings may expose local paths and sensitive metadata, so access must be limited to trusted users.
...
- [dsh-market](https://github.com/dsh-market/dsh-market): in-DSH plugin market for browsing, searching, installing, updating, and uninstalling projects listed by `awesome-dsh-plugin`; MIT and npm / Release `v1.20.2`. This version measures and persists a download region, routes the catalog, npm, GitHub packages, and images for mainland China with official-source fallback, adds a responsive theme gallery and a bounded retry when host-provided peers are absent from the Registry, discovers Windows `pnpm` installer paths, and explicitly reports when a failed pnpm patch has nevertheless written an unpatched package into the Profile. Build scripts are blocked by default, the install endpoint accepts same-origin POST only, and updates are blocked while an Agent runs and preserve a bootable state on failure, but proxy transport does not prove byte equivalence and directory inclusion is still not a security endorsement.
