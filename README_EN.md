...
- [dsh-crew](https://github.com/ZSeven-W/dsh-crew): dispatches real DSH workers from Claude Code or Codex with progress reporting, status shards, and tier policies; MIT and npm / Release `0.1.0-rc.2`. It writes to `~/.config/dsh-crew/status.d/`, and external model services may require API keys. It currently claims validation only against DSH `0.1.0-rc.6` and has no visible tests, so it is marked Early.
...
- [dsh-context](https://github.com/bowenliang123/dsh-context): shows the per-request token composition of system prompts, tool schemas, messages, injections, replies, and tool results in a Web UI Context panel and `/context` command, including compaction, pruning, and cache-hit signals; Apache-2.0 and npm / Release `0.19.2`, with DSH peer compatibility widened to `^0.1.0-rc.7` and host and client tests. It requires no external service, though the UI checks npm for a newer version at most once per hour.
...
- [dsh-ios](https://github.com/ZSeven-W/dsh-ios): brings a live iOS Simulator or USB-connected iPhone into DSH conversations with 21 tools for build-and-run, semantic UI automation, logs, backtraces, and leak checks; MIT and npm / Release `0.1.0-rc.1`, verified on DSH `0.1.0-rc.6`, with CI and smoke tests. It is still a prerelease and requires macOS with full Xcode; optional AXe may download a SHA-256-verified binary on first use, OCR compiles locally, real-device control requires a user-provided signed WebDriverAgent, and the tools can build software and act on a physical device, so it is marked Early.
...
- [dsh-win32](https://github.com/sjh9714/dsh-win32): Windows persistent shell that works inside the sandbox, plus a minimal mode and `doctor` diagnostics; MIT `v0.14.0`, verified with 98 tests and Windows restricted-token sandbox CI. It remains limited to DSH `>=0.1.0-rc.5 <0.1.0-rc.7`; the Git Bash preset requires `danger-full-access`, sandboxed use should choose the variant that downloads GPLv2 BusyBox, and Windows console processes may escalate to forced termination after graceful termination fails.
...
- [dsh-im](https://github.com/xmanrui/dsh-im): manages Feishu, WeChat, DingTalk, WeCom, QQ, Slack, Telegram, Discord, WhatsApp, and AI Office in one DSH Bundle, with multiple bots, streaming replies, workspace / Session binding, and remote approvals. MIT, npm `0.16.0` / Git tag `v0.16.0`, and Node.js 22.19+, with 85 test files and a package-verification script; Secrets and Tokens are submitted only to the local Harness Host, and management RPC is loopback-browser-only by default. Runtime use continuously receives and sends external messages and lets trusted chat users invoke models and tools; outside Telegram's optional direct-message allowlist, platform-visible users can run commands, while workspace / Session listings may expose local paths and sensitive metadata, so access must be limited to trusted users.
...
- [DShScan](https://github.com/shaoshi20/dshscan): produces rule evidence, risk scores, and install advice for DSH plugins; it can scan local content offline, or explicitly fetch GitHub/npm source, run `npm audit`, or call an external LLM. MIT and npm / Release `0.3.2`, with CI and tests for DSH-specific rules, but pinned only to DSH `0.1.0-rc.6` and iterating rapidly within one day, so marked Early. A low-risk result is not a security audit.
...
- [dsh-market](https://github.com/dsh-market/dsh-market): in-DSH plugin market for browsing, searching, installing, updating, and uninstalling projects listed by `awesome-dsh-plugin`; MIT and `v1.15.0`, adding post-install/update profile compatibility checks and one-click rollback for a single plugin. Build scripts are blocked by default, the install endpoint accepts same-origin POST only, and updates are blocked while an Agent runs and preserve a bootable state on failure, but directory inclusion is still not a security endorsement.
