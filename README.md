...
- [dsh-context](https://github.com/bowenliang123/dsh-context)：在 Web UI 的 Context 面板和 `/context` 命令中，按请求展示 System Prompt、工具 Schema、消息、注入、回复和工具结果的 Token 组成，并标出压缩、剪枝与缓存命中；Apache-2.0、npm `0.29.0` / GitHub Release `0.29.1`。`0.27`–`0.29` 新增当前上下文的工具调用 / 图片计数、注入来源与计划模式事件、净回收 Token、图片预览，以及按用户保存的趋势粒度与 Total / Diff 默认值；`0.29.1` 只修正滚动时 Turn 标签定位，但发布流程未把它推到 npm，故 `@latest` 仍是 `0.29.0`。Peer 依赖仍从 `^0.1.0-rc.7` 起并含 Host、Client 测试；无需外部服务，但 UI 会每小时至多一次查询 npm 最新版本。
...
- [dsh-im](https://github.com/xmanrui/dsh-im)：在一个 DSH Bundle 中统一管理飞书、微信、钉钉、企业微信、QQ、Slack、Telegram、Discord、WhatsApp 与 AI Office，支持多机器人、流式回复、工作区 / 会话绑定和远程审批。MIT、npm / Git Tag `2.0.1`、Node.js 22.19+，仓库含 104 个测试文件与发布包校验脚本；九个内置渠道可双向交付普通文件与图片，并统一入站暂存、路径保护、交付回执和 Turn 结束清理。2.0 新增 Telegram 原生 Rich Message、机器人消息英文模式，以及在 Discord 服务器频道首次 @ 后创建原生 Public Thread；这是 Breaking Change，需启用 Message Content Intent，并授予 Create Public Threads、Send Messages in Threads、Send Messages、Read Message History，文件交付还需 Attach Files。2.0.1 会在单一渠道激活失败时继续启动其他渠道。WhatsApp 默认只响应账号自聊；Secret / Token 仅提交给本机 Harness Host，管理 RPC 默认仅接受回环浏览器。运行时会持续收发外部消息、文件并允许获准聊天用户触发模型与工具，工作区 / 会话列表可能暴露本机路径和敏感元数据，因此只能向可信用户开放。
...
- [dsh-market](https://github.com/dsh-market/dsh-market)：DSH 内置插件市场，可浏览、搜索、安装、更新和卸载登记在 `awesome-dsh-plugin` 的项目；MIT、npm / Release `v1.21.2`。在既有实测网络分区、中国大陆路由与官方源回退基础上，本版对 Anywhere Labs Desktop 使用其可恢复安装边界，但该契约只能表达 npm `name@version`，不能覆盖 GitHub-only 项目；Tasks 面板也会显示正在更新的插件。安装后会比较整个 Profile 的 Bundle 状态并检查 CJS 语法，只报告本次操作新引入的风险；为避免误报，ESM Bundle 当前完全不做语法检查。构建脚本默认阻止，安装端点仅接受同源 POST，Agent 运行时禁止更新并在失败时保留可启动状态，但代理传输不能证明字节等价，目录收录也不代表安全背书。
