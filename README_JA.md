...
- [dsh-spec-collab](https://github.com/zx490336534/dsh-spec-collab)：製品の生要件を、Product、Engineering、双方の AI が共同レビューする Git Version 管理の Ready Spec に整理する。AI は Review Item と Candidate Patch の提出に限定され、確認と正式保存は人間が担う。Apache-2.0、npm `0.1.1`、DSH `0.1.0-rc.8` の開発依存関係を基準に Build され、23 Test を含む。ただし同日初回公開で CI、GitHub Release、独立利用の証拠がないため初期段階。`~/.dsh/spec-collab` に Collaboration Ledger と独立 Git Repository を書き込み、追加の AI Review Session を起動する。HTTP 管理面は既定で Loopback / Same-origin のみだが、花名は認証ではないため、外部公開時は別途認証と信頼できる Reverse Proxy が必要。
