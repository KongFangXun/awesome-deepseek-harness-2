...
- [dsh-context](https://github.com/bowenliang123/dsh-context)：Web UI の Context Panel と `/context` Command で、System Prompt、Tool Schema、Message、Injection、Reply、Tool Result の Request ごとの Token 構成を表示し、Compaction、Pruning、Cache-hit も示す。Apache-2.0、npm `0.29.0` / GitHub Release `0.29.1`。0.27–0.29 では現在 Context の Tool Call / Image 数、Injection Source と Plan Mode Event、正味回収 Token、Image Preview、User ごとの Trend 粒度と Total / Diff Default を追加した。0.29.1 は Scroll 中の Turn Label 位置だけを修正したが、公開 Workflow が npm へ反映できず、`@latest` は `0.29.0` のままである。Peer Dependency は引き続き `^0.1.0-rc.7` 以降で、Host / Client Test も備える。外部 Service は不要だが、UI は npm の最新版を最大 1 時間に 1 回確認する。
...
- [ModSearch](https://github.com/liustack/modsearch)：DSH に Web 検索、X 検索、ページ本文取得を追加し、構造化された根拠を返す。MIT、`v5.9.0`。同一 Engine 内の複数 API Key Rotation を追加し、Local Fetch の SSRF / DNS Rebinding 防御を維持する。検索経路によって外部 CLI、ログイン、API Key、Quota、各サービス規約への対応が必要。
...
- [dsh-im](https://github.com/xmanrui/dsh-im)：1 つの DSH Bundle で Feishu、WeChat、DingTalk、WeCom、QQ、Slack、Telegram、Discord、WhatsApp、AI Office を管理し、複数 Bot、Streaming Reply、Workspace / Session Binding、Remote Approval を提供。MIT、npm / Git Tag `2.0.1`、Node.js 22.19+ で、104 個の Test File と Package Verification Script を備える。9 個の内蔵 Channel は通常 File と Image を双方向に配信し、Inbound Staging、Path Protection、Delivery Receipt、Turn 終了時 Cleanup を共有する。2.0 は Telegram Native Rich Message、Bot Message の English Mode、Discord Server Channel で初回 @ 後に Native Public Thread を作る機能を追加した。これは Breaking Change で、Message Content Intent と Create Public Threads、Send Messages in Threads、Send Messages、Read Message History が必要で、File 配信には Attach Files も必要。2.0.1 は 1 Channel の起動失敗時にも他 Channel の起動を続ける。WhatsApp は既定で Self-chat のみ。Secret / Token は Local Harness Host のみに送られ、管理 RPC は既定で Loopback Browser のみを許可する。Runtime は外部 Message / File を継続受送信し、認可 User が Model / Tool を起動でき、Workspace / Session List は Local Path や機微 Metadata を露出し得るため、信頼できる User のみに公開する必要がある。
...
- [dsh-market](https://github.com/dsh-market/dsh-market)：`awesome-dsh-plugin` 掲載プロジェクトを DSH 内で閲覧、検索、インストール、更新、削除できる Plugin Market。MIT、npm / Release `v1.21.2`。実測 Download Region、中国本土向け Route、公式 Source Fallback に加え、利用可能なら Anywhere Labs Desktop の Recoverable Install Boundary を使うが、その Contract は npm `name@version` のみ表現でき、GitHub-only Project は対象外。Tasks Panel は実行中 Update も表示する。導入後は Profile 全体の Bundle State を比較し CJS Syntax を確認して本操作が新たに導入した Risk だけを報告するが、誤検知回避のため ESM Bundle は現在まったく Syntax Check しない。Build Script は既定で遮断され、Install Endpoint は Same-origin POST のみ。Agent 実行中は Update を防ぎ、失敗時も起動可能状態を保つが、Proxy 転送は Byte 同一性を証明せず、Directory 掲載も Security 保証ではない。
