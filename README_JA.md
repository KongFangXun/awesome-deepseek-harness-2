...
現在確認できる公式 GitHub Developer Preview の最新版は [`0.1.2-alpha.1`](https://github.com/deepseek-ai/deepseek-harness/releases/tag/dsh-v0.1.2-alpha.1) で、npm `latest` は引き続き `0.1.1-rc.2` です。この Alpha は Subagent ごとの Model 設定、ACP Automation、Multimodal / Persistent Terminal の修正を追加しました。DeepSeek Adapter Request は有効な Plugin Package 名と Version を既定で付加します（無効化可能）が、Session Log の Incremental Upload は Opt-in で既定 Off のままです。以下の各プロジェクトに記載した DSH Version は作者が実際に示した開発・テスト基準であり、最新 Preview との互換性を自動的に意味するものではありません。
...
| --- | --- | --- |
| [DeepSeek Harness Desktop (anywhere-labs)](https://github.com/anywhere-labs/dsh-desktop) | macOS (Apple Silicon) / Windows · Electron | `v2.0.2` は DSH `0.1.1-rc.2` を固定してそのまま実行し、Local Service、Tray、Recovery / Rollback、内蔵 Plugin Market を提供。上流の Breaking Change で旧 Profile や未対応 Plugin が動かなくなる可能性があるため、Upgrade 前に安全な Profile を作成し Plugin 一覧を保管する必要がある |
...
| [DeepSeek Harness Desktop App](https://github.com/vibeinging/dsh-desktop) | macOS / Windows · Electron · Community Workbench | `v0.1.9` は公式 Web / Profile Runtime 経路に Project、Git Worktree、Browser、Canvas、Site、Office 成果物、File / Folder 添付と固定 14 Bundle を追加する。macOS Apple Silicon Package は署名・公証済み。Windows x64 Package は未署名だが、GitHub Windows Runner で Install、起動、Offline Profile、Recovery、Uninstall、残存 Cleanup を検証済み。中断した Plugin Install は次回起動時に安全に Retry し、User が明示削除した Plugin は自動復元しない |
...
- [dsh-context-guard](https://github.com/GreenLv/dsh-context-guard)：要件、禁止事項、受入条件、後続修正を Task Contract として保持し、現在の成功 Evidence が現在の Contract と一致する場合だけ Goal / Turn 完了を許可し、Native Session Event から Certificate を再構築・再検証する。Apache-2.0、npm / Release `0.1.0`、DSH `0.1.1-rc.2` 固定。Repository は 104 Test と macOS / Windows の Native Isolated Acceptance を報告する。Custom Event は追加せず、Sandbox、Memory、Semantic Proof System でもないため、保守的な Command Effect 解析と Evidence Matching は見落とす場合がある。同日初版で独立利用 Evidence もないため初期段階。
...
- [oh-story-dsh](https://github.com/worldwonderer/oh-story-dsh)：13 個の小説 Skills、7 個の専門 Role、短編 Drama 制作 Workflow を DSH ネイティブの Session、Approval、3 Column の Creative Workbench に統合。MIT、npm / Release `0.1.3`、Node.js 24+、Peer Dependency は DSH `0.1.1-rc.1` 以降。Release は 34 件の自動 Test と Ubuntu / macOS / Windows CI を報告する。本版は公式 Composer の高さを再利用し、Message Stream に Scroll Safety Area と Anchor Geometry の Regression 検証を追加して、展開 Task、Streaming State、移動先 Message が Composer に隠れないようにした。Build は上流の Login / CDP Scraper と独立 Dashboard を除外する。Plugin は Model Credential を読まず独自 Listener も開かないが、呼び出し元 Agent に見える Tool の範囲で Creative Project File を読み書きして専門 Role を呼び出し、DSH Web Server に Host、Origin、Session、Path Containment、Optimistic Concurrency で保護された File Route を追加する。初回 Release は 8 月 21 日で、明示的な検証は DSH rc.1 のみ、長期保守と独立利用の証拠もまだないため初期段階。

### コンテキスト・Session・入力

- [dsh-context](https://github.com/bowenliang123/dsh-context)：Web UI の Context Panel と `/context` Command で、System Prompt、Tool Schema、Message、Injection、Reply、Tool Result の Request ごとの Token 構成を表示し、Compaction、Pruning、Cache-hit も示す。Apache-2.0、npm / Release `0.35.0`。新版は任意の深さの Subagent へ移動できる Agent Network を追加し、File Activity は PTC / Code Mode 内の呼び出し、欠損 Log、Read 行範囲も扱う。Host が `canOpenPath` を明示した場合のみ File 名を System Default Application に渡せ、Workspace 内 Path は Session Root を基準に解決し、Search Pattern など証明できない行は開けない。Peer Dependency は引き続き `^0.1.0-rc.7` 以降で、Source Test は File ごとの Coverage Gate と Bundle Smoke を備える。外部 Service は不要だが、UI は npm を最大 1 時間に 1 回確認し、公式 Registry が利用できない場合は npmmirror に Fallback する。
- [dsh-profile-settings](https://github.com/XMoon/dsh-profile-settings)：Global `settings.yaml` の上に Profile ごとの `settings.patch.yml` Overlay を追加し、Recursive Merge、`!unset`、Source Inspection、Hot Reload、Settings Page 管理を提供しながら既存の `ctx.settings` Interface を維持する。MIT、npm / Git Tag `0.1.0`、Node.js 22.6+、Peer Dependency は DSH `0.1.1-rc.2` 系に固定され、15 個の Test File を含む。最新 CI は Source Check、Build、Node 22.6 / 24 / 26 の Release Tarball Smoke Test に成功したが、Registry で `0.1.0` を取得できる状態でも全体 Workflow は npm Publish Step で失敗している。既定で Profile Overlay を書き込み、`promote` / `demote` / `migrate` は Lock、Atomic Replace、Migration Backup の下で Global / Profile Document 間の値も変更できる。同日初回公開で GitHub Release や独立利用の根拠がないため初期段階。
...
- [dsh-filesnap](https://github.com/extracurricular-ai/dsh-filesnap)：会話と Workspace File を明示的に選んだ Turn へ戻し、必ず新しい Fork で復元する。最初に Rescue Point を作成し、`/redo` で巻き戻しを反転でき、Git Branch、Commit、Stash、Worktree は変更しない。Apache-2.0、npm / Release `0.2.1`、Node.js `^22.19` または 24+、Linux / macOS / Windows の x64 / arm64 対応。CI は現行 DSH を Source Build し、10 Test File と Client Build を実行する。同梱 Rust `filesnap` Binary は Binary File、Ignored File、観測済みの Workspace 外 `ctx.fs` Path も捕捉し、復元は File を上書きするか、明示 Tombstone がある場合だけ削除する。Snapshot は現在 User の Platform Data Directory に暗号化せず保存される。Uninstall 後は Custom Event を含む Captured Session を再び開くまで再インストールが必要。同日初回公開で独立利用の根拠がないため初期段階。
...
- [dsh-plugin-appshot](https://github.com/TaurusWood/dsh-plugin-appshot)：macOS / Windows の Global Shortcut で現在の Foreground Window を撮影し、Active DSH Composer に Attachment として追加する。MIT、npm / Release `0.4.1`。新版は macOS 14+ の Native Agent Build を修正し、34 Test Path と両 Platform の Native Build を備える。Prebuilt Package は macOS App と Self-contained Windows EXE を含み、macOS では Screen Recording / Accessibility 権限が必要。Native Agent は Screenshot を永続化し Local SSE で Host へ渡す。Package Metadata に Peer Dependency がなく、開発依存は旧 DSH `0.1.0-rc.6` 固定のため、現行 Alpha 互換と高権限経路は実機再検証が必要で、初期段階。
- [dsh-ios](https://github.com/ZSeven-W/dsh-ios)：DSH の会話内に iOS Simulator または USB 接続 iPhone のライブ画面を表示し、Build / Run、Semantic UI Automation、SwiftUI Preview Hot Reload、Log、Backtrace、Leak 検査の 22 Tool を提供。MIT、Release / npm `latest` / `next` `0.1.0-rc.5`。Release Commit の Plugin Check と公開 Workflow は成功し、744 の検証 Step を自己報告している。rc.5 は混雑した WDA に短い Timeout、10 秒 Cooldown、共有 Size Cache を、停止した MJPEG Stream に 8 秒 Watchdog を追加したが、元の実機障害は Hardware 上で再現できていない。Package Peer は DSH `0.1.0-rc.6` 以降、Requirements も `>=0.1.0-rc.6` とする一方、README Header には古い rc.3 / DSH `0.1.1-rc.1` 検証表記が残り、互換 Metadata は未統一。まだ Pre-release で、完全な Xcode を備えた macOS が必要。任意の AXe は SHA-256 検証済み Binary を Download する場合があり、OCR はローカル Compile、実機操作には署名済み WebDriverAgent が必要で、Tool は Build と実機操作を実行できるため初期段階。
...
- [ModSearch](https://github.com/liustack/modsearch)：DSH に Web 検索、X 検索、ページ本文取得を追加し、構造化された根拠を返す。MIT、`v5.9.1`。同一 Engine 内の複数 API Key Rotation に対応し、Windows の自動 Route が拡張子なし Command だけを探して Native `.exe` CLI を見落とす問題を修正した。Local Fetch の SSRF / DNS Rebinding 防御を維持し、検索経路によって外部 CLI、ログイン、API Key、Quota、各サービス規約への対応が必要。
...
- [Tencent Cloud ADP for DSH](https://github.com/TencentCloudADP/Tencent-ADP-dsh-plugin)：Tencent Cloud ADP Team が管理する Model、Hunyuan AI Search、API / MCP Plugin Marketplace、Skill Plaza、公開済み ADP App の統合。MIT、npm / Release `0.1.1`、Node.js 22.19+ または 24+。Simulated HTTP Test が CI Gate で、実 Account Test は明示 Environment がある場合だけ実行する。Tool Key、SecretId / SecretKey、App ごとの AppKey は三つの別 Credential Plane で、値は DSH Credentials または Environment に置く。App / Agent / Release の Mutation は `allowMutating` 有効時だけ登録され、さらに Approval が必要。Version は新しく、外部 Cloud Account に依存し ADP Resource を変更できるため初期段階。
- [dsh-forge](https://github.com/maxmilian/dsh-forge)：Self-hosted Gitea / Forgejo の Instance、Repository、Issue、Branch、Pull Request、Actions Run、Job Log を読む Read-only Tool。MIT、npm / Release `0.3.3`、DSH Tools rc.6 と現在の rc.2 に対応し、CI は公式 Gitea / Forgejo と Runner Container で実 Integration Test を行う。最小 Read-only Token を Environment Variable で使う。npm / Prebuilt Release は Local Build 不要だが、Git Source Install は `prepare` を実行するため、Commit を確認・固定してから権限を与える。
- [dsh-sonarqube](https://github.com/maxmilian/dsh-sonarqube)：SonarQube Community Build の Quality Gate、Issue、Security Hotspot、Coverage、Duplication を Read-only で取得する。MIT、npm / Release `0.1.0`、Peer Dependency は DSH Tools rc.8 と現在の rc.2 を含み、CI、Coverage Gate、SonarQube `26.8.0.126808` の実 Instance 検証は成功。Token は Environment Variable 推奨で、返却・Log 出力しない。最初の Release であり、実 Instance に成功取得できる Hotspot Sample がなく、その経路は Mock のみで確認されているため初期段階。
- [Milvus for DSH](https://github.com/zilliztech/dsh-milvus)：Zilliz Team が管理する Milvus / Zilliz Cloud の Read-only 統合で、Collection、Schema、Exact Lookup、Scalar Query、Dense、BM25、Hybrid Search を提供。Apache-2.0、npm `0.1.3`、DSH rc.7 固定。Milvus Token と Embedding Key は Write-only DSH Credentials に保持し、Query Vector は Host Memory で生成して Milvus のみに送る。Unit Test と明示 Network-gated Integration Test はあるが GitHub Release と可視 CI はなく、外部 Endpoint、Provider Key、長期 Compatibility は運用者が検証する必要があるため初期段階。
...
- [HarnessRouter](https://github.com/HarnessRouter/harnessrouter)：Codex、Claude Code、Hermes、Pi、DSH を 1 つの Local UHP API と Console で公開する Self-hosted Container で、通常の DSH Plugin ではない。Apache-2.0、Release `v0.9.1`。Gateway / Runner / UHP Conformance Test と専用 Security 文書があり、DSH Backend は Python SDK / Runtime `0.1.0rc7` を固定導入し、内部 JSON-RPC Server が `0.1.0-rc.7` 以外なら起動を拒否する。初回起動は各 Harness CLI を Upstream から各 License の下で導入し、Docker Volume に Database、File、Workspace、Secret Material を永続化する。Console は既定で Loopback Bind だが Default Password で始まるため、外部公開前に変更と TLS が必要。Container は root で起動後に権限を落とし、Agent は Session ごとの User を使い、Provider Key は DSH Environment ではなく Loopback Relay Process に留める。新しい Project で、DSH Adapter は内部 Private Field を上書きし一部 Streaming Tool-call Delta を書き換え、固定 Runtime は MCP Client をまだ含まず、現行公式 DSH `0.1.1-rc.2` も未検証のため初期段階。
...
- [dsh-im](https://github.com/xmanrui/dsh-im)：1 つの DSH Bundle で Feishu、WeChat、DingTalk、WeCom、QQ、Slack、Telegram、Discord、WhatsApp、AI Office を管理し、複数 Bot、Streaming Reply、Workspace / Session Binding、Remote Approval を提供。MIT、npm / Release `3.0.1`、Node.js 22.19+ で、CI と Package Verification は成功。9 Channel は File / Image 配信、Inbound Staging、Path Protection、Receipt、Cleanup を共有する。3.0.0 は「IM Bot」を Top-level Settings に移して旧 Nested Entry を削除し、更新後は `dsh web` Restart が必要。3.0.1 は QQ DM の Final Markdown が一部 Client で確認済みでも表示されない問題を修正し、長文は Code Fence と GFM Table を安全に分割し、Markdown が明確に拒否された場合だけ Chunk ごとに Plain Text へ Fallback する。Discord は Message Content Intent、Thread / Message / History Permission、File には Attach Files が必要。WhatsApp は既定で Self-chat。Secret / Token は Local Host のみに送られ管理 RPC は既定で Loopback だが、認可 Chat User は Model / Tool を起動でき、Workspace / Session List が Local Path や機微 Metadata を露出し得るため信頼 User のみに限定する。
...
- [DSH Harbor](https://github.com/ZSeven-W/dsh-harbor)：導入済み Plugin の宣言と Source 検出を突合する 13 種 Capability Ledger、`file:line` Evidence、Runtime Tool / Provider / Route 所有、同一 Profile Conflict、Profile 間 Version Drift、Change Snapshot を提供。MIT、npm `next` / Git Tag `0.1.0-rc.2`、DSH `0.1.1-rc.2` 検証済みで、Node 20 / 24、Ubuntu / Windows CI と実 Package Smoke は成功。既定 Scan は Offline Read-only。Snapshot は `~/.config/dsh-harbor/` に書き、明示 Upstream Check だけが npm Registry Config を読み Network を使い、Error では Credential を Redact する。Sandbox、Install Gate、Policy Engine ではなく、Pre-release で GitHub Release や独立利用 Evidence もないため初期段階。
- [HOL Guard for DSH](https://github.com/hashgraph-online/hol-guard-plugin)：DSH の非同期 `tools/pre-execute` と最終 Monotonic Guard の両層で Tool Call に fail-closed な Local Policy 検査を適用し、曖昧な判断を 1 回限りの Native Approval に渡して Security Receipt を保存する。Apache-2.0、Release `v0.3.0`、Node.js 20+、別途 `hol-guard >=2.0.1024` が必要。Repository には 14 個の Test File があり、CI は固定した公式 DSH `0.1.0-rc.5` Source に対する実 Headless Denial Test も含む。Cloud Account は既定で不要で、Cloud Sync は任意。導入は選択した Profile を変更し、Policy は Tool 実行を阻止して Local Approval Center で判断を待つ場合がある。保護範囲は DSH `ToolRuntime` を通る Call のみで、その Pipeline 外で Plugin が直接実行する Code は制御しない。現行公式 `0.1.1-rc.2` との互換性が未証明で、最新 Runtime Version Sync Workflow も失敗しているため初期段階。
...
- [dsh-market](https://github.com/dsh-market/dsh-market)：`awesome-dsh-plugin` 掲載プロジェクトを DSH 内で閲覧、検索、Install、Update、Remove する Plugin Market。MIT、npm / Release `v1.31.1`。Region-aware Route、中国本土 Mirror、公式 Source Fallback、Anywhere Labs Desktop の Recoverable Boundary に加え、npm-only Desktop が GitHub-only Project を Install できない理由、Catalog Path が消失した場合、Restore / Rollback / Operation 後も不足する起動条件を明示する。1.29.3 は Dependency と Bundle Row の両方を Rollback し、Tasks Panel は Settings を閉じても保持される。Profile 全体の Bundle State 比較と CJS Syntax Check は継続し ESM Syntax Check は省略。Build Script は既定で遮断、Install は Same-origin POST、Agent 実行中 Update は禁止だが、Proxy は Byte 同一性を証明せず Directory 掲載も Security 保証ではない。
