...
- [dsh-win32](https://github.com/sjh9714/dsh-win32)：現在の Windows 版 DSH を診断し、安全に修復する Tool。MIT、npm `0.16.3` / 最新 GitHub Release `0.16.2` で、最新 CI は成功している。既定の `setup` は公式 Stack を置き換えず、内蔵 Persistent PowerShell と Workspace Write を検査し、確実に壊れている `koffi` Load だけを修復して Desktop Shortcut を作成できる。`doctor` は Read-only、`fix` は既知の破損版または実際に Load 失敗した導入済み `koffi` のみを修復する。注意：検証した npm 0.16.3 Package は対話式 `setup` 成功後に一度だけ Marker を書き、認証済み `gh` があれば `[y/n]` 確認なしで GitHub Star API を呼び、なければ Repository Page を開く。これは 0.16.2 Release の明示確認説明と異なる。この副作用を受け入れない場合は `doctor` のみ実行するか、`setup` 前に 0.16.2 へ固定する。旧 Git Bash / BusyBox Preset は明示的な `setup --legacy` にあり、Git Bash は `danger-full-access` が必要。Git、PowerShell、BusyBox、WSL、別の DSH Bundle は自動導入しない。
...
- [dsh-market](https://github.com/dsh-market/dsh-market)：`awesome-dsh-plugin` 掲載プロジェクトを DSH 内で閲覧、検索、Install、Update、Remove する Plugin Market。MIT、npm / Release `v1.30.0`。Region-aware Route、中国本土 Mirror、公式 Source Fallback、Anywhere Labs Desktop の Recoverable Boundary に加え、npm-only Desktop が GitHub-only Project を Install できない理由、実際の Build Failure 位置での Script Approval、Restore / Rollback / Operation 後も不足する起動条件を明示する。1.29.3 は Dependency と Bundle Row の両方を Rollback し、Tasks Panel は Settings を閉じても保持される。Profile 全体の Bundle State 比較と CJS Syntax Check は継続し ESM Syntax Check は省略。Build Script は既定で遮断、Install は Same-origin POST、Agent 実行中 Update は禁止だが、Proxy は Byte 同一性を証明せず Directory 掲載も Security 保証ではない。
