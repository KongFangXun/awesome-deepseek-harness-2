...
- [oh-story-dsh](https://github.com/worldwonderer/oh-story-dsh)：13 個の小説 Skills、7 個の専門 Role、短編 Drama 制作 Workflow を DSH ネイティブの Session、Approval、3 Column の Creative Workbench に統合。MIT、npm / Release `0.1.2`、Node.js 24+、Peer Dependency は DSH `0.1.1-rc.1` 以降。Release は 34 件の自動 Test と公式 Web UI の End-to-end 検証を報告し、Build から上流の Login / CDP Scraper と独立 Dashboard を除外する。Plugin は Model Credential を読まず独自 Listener も開かないが、呼び出し元 Agent に見える Tool の範囲で Creative Project File を読み書きして専門 Role を呼び出し、DSH Web Server に Host、Origin、Session、Path Containment、Optimistic Concurrency で保護された File Route を追加する。初回 Release は 8 月 21 日で、明示的な検証は DSH rc.1 のみ、長期保守と独立利用の証拠もまだないため初期段階。

### コンテキスト・Session・入力

- [dsh-context](https://github.com/bowenliang123/dsh-context)：Web UI の Context Panel と `/context` Command で、System Prompt、Tool Schema、Message、Injection、Reply、Tool Result の Request ごとの Token 構成を表示し、Compaction、Pruning、Cache-hit も示す。Apache-2.0、npm / Release `0.25.3`。DSH `0.1.1` の Multimodal Attachment View と Raw / Sent / Token の画像 Fact に加え、展開 Detail の Section Card 統一、Session 全体の画像数、折りたたみ Message の Image Chip、週末の Off-peak 料金修正を含む。Peer Dependency は引き続き `^0.1.0-rc.7` 以降で、Host / Client Test も備える。外部 Service は不要だが、UI は npm の最新版を最大 1 時間に 1 回確認する。
...
- [dsh-im](https://github.com/xmanrui/dsh-im)：1 つの DSH Bundle で Feishu、WeChat、DingTalk、WeCom、QQ、Slack、Telegram、Discord、WhatsApp、AI Office を管理し、複数 Bot、Streaming Reply、Workspace / Session Binding、Remote Approval を提供。MIT、npm `1.0.2` / Git Tag `v1.0.2`、Node.js 22.19+ で、89 個の Test File と Package Verification Script を備える。この Release では Feishu の REST と WebSocket の両方が明示的な HTTP(S) Proxy 設定に従う。Secret / Token は Local Harness Host のみに送られ、管理 RPC は既定で Loopback Browser のみを許可する。Runtime は外部 Message を継続的に受送信し、信頼済み Chat User が Model / Tool を起動できる。Telegram の任意 Direct-message Allowlist 以外では Platform 上で Bot が見える User が Command を実行でき、Workspace / Session List は Local Path や機微な Metadata を露出し得るため、信頼できる User のみに公開する必要がある。
