...
- [dsh-context](https://github.com/bowenliang123/dsh-context)：Web UI の Context Panel と `/context` Command で、System Prompt、Tool Schema、Message、Injection、Reply、Tool Result の Request ごとの Token 構成を表示し、Compaction、Pruning、Cache-hit も示す。Apache-2.0、npm / Release `0.26.1`。本版は Context History に Total / Diff Mode、Skill Load の表示、全高 Focus Mode を追加し、最初の Projection Data または Event 到着時に起き得た React #310 Crash を修正した。Peer Dependency は引き続き `^0.1.0-rc.7` 以降で、Host / Client Test も備える。外部 Service は不要だが、UI は npm の最新版を最大 1 時間に 1 回確認する。
...
- [dsh-win32](https://github.com/sjh9714/dsh-win32)：現在の Windows 版 DSH を診断し、安全に修復する Tool。MIT、npm / Release `v0.16.0` で最新 CI は成功している。既定の `setup` は公式 Stack を置き換えず、内蔵 Persistent PowerShell と Workspace Write を検査し、確実に壊れている `koffi` Load だけを修復して Desktop Shortcut を作成できる。`doctor` は Read-only、`fix` は既知の破損版または実際に Load 失敗した導入済み `koffi` のみを修復する。旧 Git Bash / BusyBox Preset は明示的な `setup --legacy` に移り、Git Bash は引き続き `danger-full-access` が必要。Git、PowerShell、BusyBox、WSL、別の DSH Bundle を自動導入しない。
...
- [dsh-im](https://github.com/xmanrui/dsh-im)：1 つの DSH Bundle で Feishu、WeChat、DingTalk、WeCom、QQ、Slack、Telegram、Discord、WhatsApp、AI Office を管理し、複数 Bot、Streaming Reply、Workspace / Session Binding、Remote Approval を提供。MIT、npm `1.3.0` / Git Tag `v1.3.0`、Node.js 22.19+ で、100 個の Test File と Package Verification Script を備える。9 個の内蔵 Channel は通常 File を双方向に配信でき、Inbound File の Prefetch、Workspace Staging、Path Protection、Failure Message、Turn 終了時 Cleanup が統一された。既存・新規の WhatsApp Bot は既定で Self-chat のみに応答し、指定 Contact または Open Response に変更できる。Secret / Token は Local Harness Host のみに送られ、管理 RPC は既定で Loopback Browser のみを許可する。Runtime は外部 Message と File を継続的に受送信し、許可された Chat User が Model / Tool を起動できる。File Type、数、Size、Access Policy は各 Platform が決め、Workspace / Session List は Local Path や機微な Metadata を露出し得るため、信頼できる User のみに公開する必要がある。
...
- [dsh-market](https://github.com/dsh-market/dsh-market)：`awesome-dsh-plugin` 掲載プロジェクトを DSH 内で閲覧、検索、インストール、更新、削除できるプラグインマーケット。MIT、npm / Release `v1.20.2`。本版は実測 Network から Download Region を選んで保持し、中国本土向けに Catalog、npm、GitHub Package、Image を Route しつつ失敗時は公式 Source へ戻す。Responsive Theme Gallery、Host 提供だが Registry にない Peer の限定 Retry、Windows の `pnpm` Install Path 検出も追加し、pnpm Patch が失敗しても未 Patch Package を Profile に書いた場合は状態変化を明示する。Build Script は既定で遮断され、Install Endpoint は Same-origin POST のみ。Agent 実行中は Update を防ぎ、失敗時も次回起動可能な状態を保つが、Proxy 転送は Byte 同一性を証明せず、Directory 掲載も Security 保証ではない。
