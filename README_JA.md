...
- [dsh-context](https://github.com/bowenliang123/dsh-context)：Web UI の Context Panel と `/context` Command で、System Prompt、Tool Schema、Message、Injection、Reply、Tool Result の Request ごとの Token 構成を表示し、Compaction、Pruning、Cache-hit も示す。Apache-2.0、npm / Release `0.33.1`。0.31 以降は符号付き Delta で隣接 Request 間の増加・回収を表示し、0.32 は各 Step に User / In / Response Brief と元 Message への Link を追加、0.33 は File ごとの Read / Write / Search 回数、推定行増減と正確な Tool Result Link を持つ File Activity を追加し、0.33.1 は Activity、時刻、Path、用途 Sort を加えた。Peer Dependency は引き続き `^0.1.0-rc.7` 以降で、Source Test は File ごとの Coverage Gate と Bundle Smoke を備える。外部 Service は不要だが、UI は npm の最新版を最大 1 時間に 1 回確認する。
...
- [dsh-plugin-appshot](https://github.com/TaurusWood/dsh-plugin-appshot)：macOS / Windows の Global Shortcut で現在の Foreground Window を撮影し、Active DSH Composer に Attachment として追加する。MIT、npm / Release `0.4.0`、34 Test Path と両 Platform の Native Build を備える。Prebuilt Package は macOS App と Self-contained Windows EXE を含み、macOS では Screen Recording / Accessibility 権限が必要。Native Agent は Screenshot を永続化し Local SSE で Host へ渡す。Package Metadata に Peer Dependency がなく、開発依存は旧 DSH `0.1.0-rc.6` 固定のため、現行 rc.2 互換と高権限経路は実機再検証が必要で、初期段階。
